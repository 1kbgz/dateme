...
makeup, while `max_skip_gap` can turn long occurrence gaps into query errors.
`makeup` can also vary by excluded weekday, and `makeup_only_on` can restrict
makeup target weekdays. Makeup can also be constrained to the same week,
